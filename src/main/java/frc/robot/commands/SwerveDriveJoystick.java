...
public class SwerveDriveJoystick extends Command {
    ...
    private final Supplier<Double> x, y, z; // <-- Suppliers for joystick inputs (X, Y, Z axes).
    private final Supplier<Boolean> fieldRelative; // <-- Supplier for field-relative mode.
    private final SlewRateLimiter xLimiter, yLimiter, zLimiter; // <-- Rate limiters for joystick inputs.
    ...
    public void execute() {
        ...
        xSpeed = Math.abs(xSpeed) > OperatorConstants.JOYSTICK_DEADZONE ? xSpeed : 0.0; // <-- Filters X-axis input.
        ySpeed = Math.abs(ySpeed) > OperatorConstants.JOYSTICK_DEADZONE ? ySpeed : 0.0; // <-- Filters Y-axis input.
        zSpeed = Math.abs(zSpeed) > OperatorConstants.JOYSTICK_DEADZONE ? zSpeed : 0.0; // <-- Filters Z-axis input.
